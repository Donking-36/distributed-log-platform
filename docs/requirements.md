...
本地 Kafka 短时不可用验收限定为：在核对 StatefulSet、PVC、Cluster ID 和起始
位点后，将单 Broker 受控缩容 1→0；仅在 Broker Pod 缺席且 Filebeat output
语义检查确认不可达后生成唯一批次，再恢复为 1，并按四主题有界位点对账。它证明
声明窗口内的恢复，不证明多节点故障转移、无限中断或节点磁盘丢失恢复。
...
| 延迟 | 可选记录 `@timestamp`→`ingested_at` 的 p50/p95/p99，作为定位瓶颈的辅助数据，不设置硬性阈值；页面查询/刷新另测 | 不以单次截图代替统计结果，也不把摄取延迟冒充页面可视化延迟 |
| 吞吐 | 硬性门禁：在固定本机资源、固定事件大小和固定持续时长下，完整链路持续处理速率必须达到每秒 1000 条以上；同时证明输入数等于 Elasticsearch 最终唯一文档数、处理错误数为 0、消费者组最终 LAG 为 0 | 不把单节点 Minikube 结果外推为生产容量或未验证的发布保证 |
...
- 固定本机环境下的完整链路吞吐验收达到每秒 1000 条以上，且输入数等于最终唯一
  文档数、处理错误数为 0、消费者组最终 LAG 为 0。
