...
Elasticsearch 数据源。数据源和仪表盘都由 ConfigMap 文件预置；本地单副本
...
2026-08-05 在 WSL2 Minikube 4 CPU/6 GiB、约 193.4 字节事件、两个日志源各
60000 条/1ms 的条件下，完整链路从创建 Job 到 ES 精确计数和 LAG=0 共用时
83.877 秒，达到 1430.66 条/秒；输入数与 ES 唯一文档数均为 120000，处理错误
为 0，处理器 0 重启。该证据不外推为生产容量。
